# Data Subject Requests for the GDPR #

This document provides guidance for the completion of Data Subject Requests (DSRs) under the General Data Protection Regulation (GDPR) using Microsoft products and services. third-party products and services accessed through Microsoft account authentication, any data subject requests should be directed to the applicable third party.

Several processes may be involved completing a DSR, subject to your organization’s GDPR-compliance rules.  
1. **Discovery**. The process of determining what data is needed to complete a DSR.
1. **Access**. Retrieval and potential transmission to the data subject of discovered information.
1. **Rectify**. Implement changes or other requested personal data changes.
1. **Restrict**. Changing the access or processing of persona data by restricting access, or removing data from the Microsoft cloud.
1. **Export**. Providing a “structured, commonly used, machine-readable format” of personal data to the data subject, as provided by the GDPR’s “right of data portability”.
1. **Delete**. Permanent removal of personal data from the Microsoft cloud.

An outline for applying these processes across Microsoft services and products, along with further resources links, is given below. A DSR will typically call on three data types. Performing DSR tasks will vary depending on the data [type][DSRdataTypes]. There are three basic types you will encounter.  
1. **Customer Data**. Data produced and stored in Office 365 in the day-to-day operations of your business.
1. **Insights generated by Microsoft Products**. [Insights][DSRinsights] may be generated by services (MyAnalytics, etc.)
1. **System-generated logs**. Logs and related data generated by Microsoft may contain data deemed personal under GDPR's definition of "personal data."

Completing a DSR is outlined below, addressed by data type and process (Discovery, Export, etc.) Refer to [DSR References](#dsr-references) or click on the subject title (Discover, etc.) for additional details regarding your Microsoft products and services implementation.

1. **Customer Data**  
    * [**Discover**](#discover). Searching data varies across Microsoft products and services, as well as the specifics of your configuration.
    * [**Access**](#access). An access DSR can be completed via a copy of the actual document, appropriately redacted version, or a screenshot of the portions you have deemed appropriate to share.
    * [**Rectify**](#rectify). Honoring a DSR to rectify personal data may include editing, redacting, or removing personal data. In some cases the data subject may make the requested change.
    * [**Restrict**](#restrict). There are several ways to restrict personal data processing: temporary removal of DSR-specific data, access restrictions to Microsoft products and services, removal of application licenses or preventing user sign in.
    * [**Export**](#export). Providing a “structured, commonly used, machine-readable format” of personal data to the data subject, as provided by the GDPR’s “right of data portability”.
    * [**Delete**](#delete). The “right to erasure” by the removal of personal data from an organization’s Customer Data is a key protection in the GDPR. This includes both recoverable (“soft”) and non-recoverable (“hard”) deletion of specific data within a document, deleting entire documents or files, deleting sites (from SharePoint, etc.) or user accounts.

2. **Insights generated in Office 365**. Office 365 includes online services that provide insights to users and organizations that use them. Data generated by these services may produce personal data relevant to a DSR. The list below provides details regarding elements of the DSR processes specific to those services.  
a. **Individual User Data**
    1. [Delve][DSRdelve]
    2. [MyAnalytics][DSRmyAnalytics]

b. **Organization data**  
    1. [Workplace Analytics][DSRworkAnalytics]

3. **DSRs for system-generated logs**. Restricting or rectifying data in system-generated logs is not supported. Data in system-generated logs constitutes factual actions conducted within the Microsoft cloud and diagnostic data; modifications would compromise the historical record of actions and increase fraud and security risks. Microsoft provides the ability to access, export, and delete system-generated logs that may be necessary to complete a DSR. Examples of such data may include:  
    * Product and service usage data such as user activity logs
    * User search requests and query data
    * Data generated by product and services resulting from system functionality and interaction by users or other systems.  
Implementation specific details are given in [DSR References](#dsr-references).

Deleting a user’s account will not remove system-generated logs for Yammer and Kaizala. To remove the data from these applications, see one of the following:
 * [Manage GDPR data subject requests in Yammer Enterprise][DSRyammer]
 * [Export or delete a user's organizational data in Kaizala][DSRkaizala]

In some national clouds, a global IT Administrator will need to delete system-generated logs.

If your organization or users engage with Microsoft to receive support related to Microsoft products and services some of this data may contain personal data. For more information, see [Microsoft Support and Professional Services Data Subject Requests for the GDPR][DSRMSPS].

In some [circumstances][DSRuserInit] your organization’s users may access Microsoft products or services for which Microsoft is the data controller. In those cases, your users will need to initiate their own DSRs directly to Microsoft, and Microsoft will fulfill the requests directly to the user.

## DSR References ##

### Discover ###
* _Office 365_. The Content Search eDiscovery tool can be used to search Office 365 data.  
* _Azure Active Directory (AAD)_. You can discover personal data using the [Azure portal][DSRazurePortal].  
* _Intune_. A DSR against an Intune account _associated with an AAD tenant **will only**_ pertain to data within the tenant. (This is true of all tenant services.) Details in [Step 1: Discover][DSRintuneDisc].
* _Dynamics 365_. Dynamics 365 provides multiple methods to search for personal data such as: [Advanced Find Search][DSRD365advSearch], [Relevance Search][DSRD365relevance], and [Search for Records][DSRD365records].  
* _Visual Studio Family (VSF)_. If you are registered using an Azure backed tenant, follow the instructions for AAD above. If registered without Azure tenant backing use the [web-based Microsoft Privacy Response Center][DSRVSFprivacyResp].  
* _Azure DevOps_. AAD tenant accounts can follow the instructions above. Users implementing a MSA identity can use this [Privacy Request site][DSRADOprivacyReq].  
* _Microsoft Support and Professional Services_. Follow [Step 1: Discover][DSRMSPSstep1].

### Access ###
* _Office 365_. An access DSR may include edited electronic or physical copy of the data.  
* _Azure Active Directory (AAD)_. Follow the link given for details on providing access in AAD.  
* _Intune_. For additional details, refer to [Step 2: Access][DSRintuneAccess].
* _Dynamics 365_. Refer to [Providing a copy of customer][DSRD365acces] data for details.  
* _Visual Studio Family (VSF)_. Information can be found in [How users can control personal data][DSRVSFaccess].  
* _Azure DevOps_. see Visual Studio Family above regarding AAD tenant accounts. MSA identity users may use the [Privacy Request site][DSRADOaccess].  
* _Microsoft Support and Professional Services_. Microsoft Support and Professional Services: For details, see [Step 2: Access][DSRMSPSaccess].

### Rectify ###
* _Office 365_. Making changes to personal data may be made by the data subject, controller or IT administrator.  
* _Azure Active Directory (AAD)_. Enterprise customers may manage DSR rectify requests, including limited editing features per given Microsoft services.
* _Intune_. For details relevant to Intune, see [Step 3: Rectify][DSRintuneRectify].
* _Dynamics 365_. Dynamics 365 offers several methods to rectify personal data, depending on the tools you are employing.
* _Visual Studio Family (VSF)_. Personal data activities vary according to the VSF implementation. For specific details, see the appropriate section [here][DSRVSFrestrict].  
* _Azure DevOps_. There is no reference to data rectification in the given reference.  
* _Microsoft Support and Professional Services_. Microsoft may rectify data or delete it from the support systems and request the customer resubmit it in corrected format.

### Restrict ###
* _Office 365_. Actions that satisfy a restriction DSR in Office are given [here][DSRO365restrict].  
* _Azure Active Directory (AAD)_. Azure Portal and pre-existing application programming interfaces (APIs) or user interfaces (UIs) allow the tenant administrator to manage DSRs.
* _Intune_. For details, see [Processing personal data][DSRintuneProcessing].
* _Dynamics 365_. An easy way to restrict personal data processing is to extract and store separately the relevant data. For details see [Restrict data processing of Customer Data][DSRD365restrict].
* _Visual Studio Family (VSF)_. Personal data activities vary according to the VSF implementation. For specific details, see the appropriate section [here][DSRADOrestrict].  
* _Azure DevOps_. There is no reference to data rectification in the given link.  
* _Microsoft Support and Professional Services_. The customer may at any time close an engagement or request the engagement be closed, preventing any data processing. For details, see [Step 4: Restrict][DSRMSPSrestrict].

### Export ###
* _Office 365_. There are several ways to export of data in Office 365.
* _Azure Active Directory (AAD)_. Both portal and in-product experiences allow an administrator to manage export requests. A tenant administrator must export user data from a tenant. For details, refer to [Step 6: Export][DSRAADexport].
* _Intune_. Follow instructions for AAD (no reference?)
* _Dynamics 365_. There are several ways to export data in Dynamics 365.
* _Visual Studio Family (VSF)_. Microsoft will create .zip archives, as explained in [Export][DSRVSFexport].  
* _Azure DevOps_. Controllers may export data based on login (AAD or MSA.) See [Export Azure DevOps][DSRADOexport] data for specifics.  
* _Microsoft Support and Professional Services_. Microsoft Support Data may include information or files that can be returned to you for communication to another controller. This process is described in [Step 6: Export][DSRMSPSexport].

### Delete ###
* _Office 365_. Details on record deletion can be found in [Deleting person data][DSRO365delete].
* _Azure Active Directory (AAD)_. The Azure portal can be used to delete a user and the associated personal information, and system-generated logs.
* _Intune_. For details, see [Delete end user personal data][DSRintuneDelete].
* _Dynamics 365_. Information on deleting customer data is at [Deleting customer data][DSRD365delete].
* _Visual Studio Family (VSF)_. Actions vary according to the VSF implementation you are using.  
* _Azure DevOps_. See [Delete Azure DevOps][DSRADOdelete] data for specifics.  
* _Microsoft Support and Professional Services_. Removing personal data includes deleting entire engagements, documents, files or specific data within an engagement, document or file. Details are given in [Step 5: Delete][DSRMSPSdelete].

### System-generated logs ###
* _Office 365_. Specific details can be found in [Responding to DSRs for System-generated Logs][DSRO365logs].
* _Azure Active Directory (AAD)_. You may execute DSRs within AAD, and your tenant. Refer to [Executing DSRs against System-generated logs][DSRAADlogs].
* _Intune_. Audit logs may contain personal data from user devices enrolled in Intune. Admins can't delete audit logs. See [Part 2: System-generated Logs][DSRintuneLogs].
* _Dynamics 365_. Details on accessing system-generated logs are in [Part 2][DSRD365logs]
* _Visual Studio Family (VSF)_. There is no specific information here.  
* _Azure DevOps_.  There is no specific information here.  
* _Microsoft Support and Professional Services_. Microsoft's response to DSRs is detailed in [this document][DSRMSPSlogs].


[DSRdataTypes]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-office365?toc=/microsoft-365/enterprise/toc.json#how-to-use-this-guide
[DSRinsights]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-office365?toc=/microsoft-365/enterprise/toc.json#part-2-responding-to-dsrs-with-respect-to-insights-generated-by-office-365

[DSRdelve]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-office365?toc=/microsoft-365/enterprise/toc.json#delve
[DSRmyAnalytics]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-office365?toc=/microsoft-365/enterprise/toc.json#myanalytics
[DSRworkAnalytics]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-office365?toc=/microsoft-365/enterprise/toc.json#workplace-analytics
[DSRyammer]: https://docs.microsoft.com/yammer/manage-security-and-compliance/gdpr-requests-in-yammer-enterprise
[DSRkaizala]: https://docs.microsoft.com/office365/kaizala/export-or-delete-a-user-s-data
[DSRMSPS]: https://docs.microsoft.com/microsoft-365/compliance/gdpr-dsr-prof-services
[DSRuserInit]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-office365?toc=/microsoft-365/enterprise/toc.json

[DSRazurePortal]: https://portal.azure.com/
[DSRintuneDisc]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-intune?toc=/microsoft-365/enterprise/toc.json#step-1-discover
[DSRD365advSearch]: https://docs.microsoft.com/en-us/dynamics365/customer-engagement/basics/save-advanced-find-search
[DSRD365relevance]: https://docs.microsoft.com/en-us/dynamics365/customer-engagement/basics/relevance-search-results
[DSRD365records]: https://docs.microsoft.com/en-us/dynamics365/customer-engagement/basics/search-records
[DSRVSFprivacyResp]: https://aka.ms/userprivacysite
[DSRADOprivacyReq]: https://www.microsoft.com/concern/privacyrequest-msa
[DSRMSPSstep1]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-prof-services?toc=/microsoft-365/enterprise/toc.json#step-1-discover

[DSRintuneAccess]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-intune?toc=/microsoft-365/enterprise/toc.json#step-2-access
[DSRD365acces]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-dynamics365?toc=/microsoft-365/enterprise/toc.json#providing-a-copy-of-customer-data
[DSRVSFaccess]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-visual-studio-family?toc=/microsoft-365/enterprise/toc.json
[DSRADOaccess]: https://www.microsoft.com/concern/privacyrequest-msa
[DSRMSPSaccess]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-prof-services?toc=/microsoft-365/enterprise/toc.json#step-2-access

[DSRintuneRectify]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-intune?toc=/microsoft-365/enterprise/toc.json#step-3-rectify

[DSRO365restrict]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-office365?toc=/microsoft-365/enterprise/toc.json#responding-to-dsr-restriction-requests
[DSRintuneProcessing]: https://docs.microsoft.com/intune/privacy-data-store-process#processing-personal-data
[DSRD365restrict]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-dynamics365#restricting-the-processing-of-customer-data
[DSRVSFrestrict]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-visual-studio-family?toc=/microsoft-365/enterprise/toc.json
[DSRADOrestrict]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-vsts?toc=/microsoft-365/enterprise/toc.json
[DSRMSPSrestrict]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-prof-services?toc=/microsoft-365/enterprise/toc.json#step-4-restrict

[DSRAADexport]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-azure?toc=/microsoft-365/enterprise/toc.json#step-6-export
[DSRVSFexport]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-visual-studio-family?toc=/microsoft-365/enterprise/toc.json
[DSRADOexport]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-vsts?toc=/microsoft-365/enterprise/toc.json#export-azure-devops-data
[DSRMSPSexport]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-prof-services?toc=/microsoft-365/enterprise/toc.json#step-6-export

[DSRO365delete]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-office365?toc=/microsoft-365/enterprise/toc.json#deleting-personal-data
[DSRintuneDelete]: https://docs.microsoft.com/intune/privacy-data-audit-export-delete#delete-end-user-personal-data
[DSRD365delete]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-dynamics365?toc=/microsoft-365/enterprise/toc.json
[DSRADOdelete]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-vsts?toc=/microsoft-365/enterprise/toc.json#delete-azure-devops-data
[DSRMSPSdelete]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-prof-services?toc=/microsoft-365/enterprise/toc.json#step-5-delete

[DSRO365logs]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-office365?toc=/microsoft-365/enterprise/toc.json#part-3-responding-to-dsrs-for-system-generated-logs
[DSRAADlogs]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-azure?toc=/microsoft-365/enterprise/toc.json#executing-dsrs-against-system-generated-logs
[DSRintuneLogs]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-intune?toc=/microsoft-365/enterprise/toc.json
[DSRD365logs]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-dynamics365?toc=/microsoft-365/enterprise/toc.json
[DSRMSPSlogs]: https://docs.microsoft.com/en-us/microsoft-365/compliance/gdpr-dsr-prof-services?toc=/microsoft-365/enterprise/toc.json